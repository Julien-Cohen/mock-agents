...
class SampleAgentExecutor(AgentExecutor):
    ...
    async def execute(
        ...
        logger.info(
            '[ConsensusAgentExecutor] Processing message %s for task %s (context: %s)',
        ...
        if task_id not in self.running_tasks:
            logger.info(
                '[ConsensusAgentExecutor] Task Error',
            )
            return
        ...
        explanation_artifact = Artifact(
            artifact_id=str(uuid.uuid4()),
            name='explanation',
            parts=[Part(text="The consensus is positive to me.")],
        )
        await event_queue.enqueue_event(TaskArtifactUpdateEvent(
            context_id=task.context_id,
            task_id=task.id,
        ...
        logger.info(
            '[ConsensusAgentExecutor] Task %s finished with state: completed',
            task_id,
        )
...
async def serve(
    ...
    agent_card = AgentCard(
        ...
        description='A mock agent to test consensus (positive answer).',
    ...
    logger.info('Starting Consensus Agent servers:')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='A2A Consensus agent server')
